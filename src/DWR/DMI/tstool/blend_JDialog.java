// ----------------------------------------------------------------------------
// blend_JDialog - editor for blend()
// ----------------------------------------------------------------------------
// Copyright:	See the COPYRIGHT file.
// ----------------------------------------------------------------------------
// History: 
//
// 08 May 2001	Steven A. Malers, RTi	Copy divide_Dialog and modify.
...
private JList		__independent_JList =null;// Field for independent time
						// series identifier
private SimpleJComboBox	__method_JComboBox = null;// Choice for blend methods.
...
{	String alias = __alias_JComboBox.getSelected();
	...
	if ( alias.equalsIgnoreCase(independent) ) {
		warning +=
			"\nTime series to blend \"" + alias +
			"\" is the same.\n"+
			"as the time series to blend by \"" + independent +
			"\".\n" + "Correct or Cancel.";
	}
	if ( warning.length() > 0 ) {
		__error_wait = true;
...
{	__command_Vector = command;
	...
	if ( size == 0 ) {
		Message.printWarning ( 1, "blend_JDialog.initialize",
		"You must define time series before inserting a blend() "+
		"command." );
		response ( 0 );
	}
...
The command is like:
<pre>
blend(Alias,IndependentID,method)
</pre>
*/
...
{	String alias = "";
	...
	if ( __first_time ) {
		...
		if ( (v != null) && (v.size() >= 4) ) {
			...
			method_type = ((String)v.elementAt(3)).trim();
		}
		...
		if ( (v != null) && (v.size() > 0) ) {
			...
			if (	JGUIUtil.isSimpleJComboBoxItem(
				__method_JComboBox, method_type,
				JGUIUtil.NONE, null, null ) ) {
				__method_JComboBox.select ( method_type );
			}
			else {	Message.printWarning ( 1,
				"blend_JDialog.refresh", "Existing " +
	...
	method_type = __method_JComboBox.getSelected();
