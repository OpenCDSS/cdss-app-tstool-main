// ----------------------------------------------------------------------------
// TSreadNwsCard_JDialog - editor for TS x = readNwsCard()
// ----------------------------------------------------------------------------
...
/**
The TSreadNwsCard_JDialog edits the TS x = readNwsCard() command.
...
*/
public class TSreadNwsCard_JDialog extends JDialog
...
/**
TSreadNwsCard_JDialog constructor.
...
*/
public TSreadNwsCard_JDialog (	JFrame parent, PropList app_PropList,
				Vector command, Vector tsids )
{	super(parent, true);
	initialize ( parent, "Edit TS x = readNwsCard() Command",
...
{	String file = __file_JTextField.getText().trim();
	...
	String routine = "TSreadNWSCard_JDialog.checkInput";
	String warning = "";
	...
	try {	String adjusted_path = IOUtil.adjustPath ( __working_dir, file);
		...
		if ( !f.exists() ) {
			warning +=
			"The NWS Card file does not exist:\n" +
			"    " + adjusted_path + "\n" +
		  	"Correct or Cancel.";
		}
		f = null;
	...
	if (	!analysis_period_start.equals("*") &&
		...
		try {	startdate = DateTime.parse( analysis_period_start);
		}
		catch ( Exception e ) {
			warning +=
...
try {
	...
	int y = 0;
		...
		"Read a single time series from a NWS Card format file."),
...
{	String alias = "";
	...
	if ( __first_time ) {
		...
		if ( ((String)__command_Vector.elementAt(0)).indexOf('=')>=0){
			...
			// Assume TS X = readNwsCard(file,units,start,end)
