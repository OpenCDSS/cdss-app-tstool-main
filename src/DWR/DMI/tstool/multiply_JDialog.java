// ----------------------------------------------------------------------------
// multiply_JDialog - editor for multiply()
// ----------------------------------------------------------------------------
...
{	String alias = __alias_JComboBox.getSelected();
	...
	if ( alias.equalsIgnoreCase(independent) ) {
		warning +=
			"Time series to multiply \"" + alias +
			"\" is the same.\n"+
			"as the time series to multiply by \"" + independent +
			"\".\n" + "Correct or Cancel.";
	}
	if ( warning.length() > 0 ) {
		__error_wait = true;
...
{	__parent_JFrame = parent;
	...
	int y = 0;
		...
		"Multiply one time series by another.  Missing data in either" +
		" sets the result to missing." ), 
		0, y, 7, 1, 0, 0, insetsTLBR, gbc.NONE, gbc.WEST);
        JGUIUtil.addComponent(main_JPanel, new JLabel (
		"Right-click on \"Time Series to Multiply by\" to convert to" +
		" TEMPTS." ), 
		0, ++y, 7, 1, 0, 0, insetsTLBR, gbc.NONE, gbc.WEST);

        JGUIUtil.addComponent(main_JPanel, new JLabel (
		"Time Series to Multiply:" ), 
	...
	if ( size == 0 ) {
		Message.printWarning ( 1, "multiply_JDialog.initialize",
		"You must define time series before inserting a multiply() "+
		"command." );
		response ( 0 );
	}
