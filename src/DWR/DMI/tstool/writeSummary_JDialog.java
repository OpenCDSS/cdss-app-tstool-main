// ----------------------------------------------------------------------------
// writeSummary_JDialog - editor for writeSummary()
// ----------------------------------------------------------------------------
// Copyright:	See the COPYRIGHT file.
// ----------------------------------------------------------------------------
// History: 
//
// 11 Sep 2001	Steven A. Malers, RTi	Initial version (copy and modify
//					writeStateMod_Dialog).
// 2002-04-23	SAM, RTi		Clean up dialog.
// 2003-12-03	SAM, RTi		Update to Swing.
// 2004-01-28	SAM, RTi		* Take out note about being applicable
//					  only for frost dates.
//					* Add note about output period being
//					  used for output.
...
*/
public writeSummary_JDialog (	JFrame parent, PropList app_PropList,
				Vector command, Vector tsids )
{	super(parent, true);
	initialize ( parent, "Edit writeSummary() Command", app_PropList,
		command, tsids );
}
		...
		if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
			...
			if (path != null) {
				if ( fc.getFileFilter() == sff ) {
					// Enforce extension...
					path = IOUtil.enforceFileExtension(
						path, "txt");
				}
...
{	String file = __file_JTextField.getText().trim();
	String routine = "writeSumary_JDialog.checkInput";
	...
	try {	String adjusted_path = IOUtil.adjustPath ( __working_dir, file);
		...
		if ( !f2.exists() ) {
			warning +=
			"\nThe summary file parent directory does " +
			"not exist:\n    " + adjusted_path + "\n" +
		  	"Correct or Cancel.";
		}
		f = null;
	...
	if ( warning.length() > 0 ) {
		__error_wait = true;
		Message.printWarning ( 1, "writeSummary_JDialog", warning );
	}
}
...
{	__parent_JFrame = parent;
	...
	int y = 0;
		...
		"Write in-memory time series to a summary format file," +
	...
	if ( __working_dir != null ) {
		...
		0, ++y, 7, 1, 0, 0, insetsTLBR, gbc.NONE, gbc.WEST);
        JGUIUtil.addComponent(main_JPanel, new JLabel (
		"writeSummary() will use the output period for output," +
		" if defined."),
		0, ++y, 7, 1, 0, 0, insetsTLBR, gbc.NONE, gbc.WEST);
...
{	String file = "";
	...
	__command_JTextField.setText("writeSummary(\"" + file + "\")" );
