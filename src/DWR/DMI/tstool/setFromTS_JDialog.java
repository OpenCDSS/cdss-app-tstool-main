// ----------------------------------------------------------------------------
// setFromTS_JDialog - editor for setFromTS()
// ----------------------------------------------------------------------------
...
//					fillFromTS_JDialog().
// 2003-10-06	SAM, RTi		* Change name from setFromTS_JDialog to
//					  setFromTS_JDialog.
//					* Convert to Swing.
...
private SimpleJComboBox	__transfer_JComboBox =null;
						// Indicates how to transfer
						// data.
...
{	__command_Vector = command;
	...
	if ( size == 0 ) {
		Message.printWarning ( 1, "setFromTS_JDialog.initialize",
		"You must define time series before inserting a setFromTS() "+
		"command." );
		response ( 0 );
	}
...
The command is like:
<pre>
setFromTS(Alias,AnalysisStart,AnalysisEnd)
</pre>
*/
...
{	String alias = "";
	...
	if ( __first_time ) {
		...
		if ( (v != null) && (v.size() >= 5) ) {
			...
			if (	JGUIUtil.isSimpleJComboBoxItem(
				...
				"setFromTS_JDialog.refresh", "Existing " +
	...
	transfer = __transfer_JComboBox.getSelected();
	...
	__command_JTextField.setText(command+"(" + alias + "," +
		...
		analysis_period_end + ",TransferData=" + transfer+ ")" );
