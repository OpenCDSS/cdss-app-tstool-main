// ----------------------------------------------------------------------------
// TSreadDateValue_JDialog - editor for TS x = readDateValue()
// ----------------------------------------------------------------------------
// Copyright:	See the COPYRIGHT file.
// ----------------------------------------------------------------------------
// History: 
//
// 2002-04-25	Steven A. Malers, RTi	Initial version (copy and modify
//					readUsgsNwis).
// 2002-04-28	SAM, RTi		Fix bug where warnings were getting
//					printed as dates are entered.
// 2002-05-26	SAM, RTi		Fix bug where dates with hours were not
//					parsing correctly.
// 2003-10-05	SAM, RTi		* Convert to Swing.
...
/**
The TSreadDateValue_JDialog edits the TS x = readDateValue() command.
...
*/
public class TSreadDateValue_JDialog extends JDialog
...
/**
TSreadDateValue_JDialog constructor.
...
{	String file = __file_JTextField.getText().trim();
	...
	String routine = "TSreadDateValue_JDialog.checkInput";
...
{	__command_Vector = command;
	...
	int y = 0;
		...
		"Specifying \"Identifier/Alias to Read\" allows"),
		0, y, 7, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
       	JGUIUtil.addComponent(main_JPanel, new JLabel (
		"reading from a file containing multiple time series " +
...
{	String alias = "";
	...
	if ( __first_time ) {
		...
		if ( ((String)__command_Vector.elementAt(0)).indexOf('=')>=0){
			...
			// Assume TS X =readDateValue(file,TSID,units,start,end)
	...
	if (	!analysis_period_start.equals("*") &&
		...
		!analysis_period_start.equalsIgnoreCase("QueryStart") &&
		!analysis_period_start.equalsIgnoreCase("QueryEnd") ) {
		try {	DateTime startdate = DateTime.parse(analysis_period_start);
			if (	(startdate == null) ||
				(startdate.getYear() == 0) ) {
				startdate = null;
			...
			if ( analysis_period_start.length() > 0 ) {
				// Only print the warning if not the initial
				// blank value.
				Message.printWarning ( 1,
				"TSreadDateValue_JDialog.refresh",
	...
	if (	!analysis_period_end.equals("*") &&
		...
		try {	DateTime enddate = DateTime.parse(analysis_period_end);
			if (	(enddate == null) ||
				(enddate.getYear() == 0) ) {
				enddate = null;
				throw new Exception ( "bad end date" );
			}
			enddate = null;
		}
		catch ( Exception e ) {
			if ( analysis_period_end.length() > 0 ) {
				// Only print the warning if not the initial
				// blank value.
				Message.printWarning ( 1,
				"TSreadDateValue_JDialog.refresh",
