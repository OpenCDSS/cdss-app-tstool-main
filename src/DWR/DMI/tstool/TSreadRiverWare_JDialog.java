// ----------------------------------------------------------------------------
// TSreadRiverWare_JDialog - editor for TS x = readRiverWare()
// ----------------------------------------------------------------------------
// Copyright:	See the COPYRIGHT file.
// ----------------------------------------------------------------------------
// History: 
//
// 2002-06-03	Steven A. Malers, RTi	Initial version (copy and modify
...
/**
The TSreadRiverWare_JDialog edits the TS x = readRiverWare() command.
...
*/
public class TSreadRiverWare_JDialog extends JDialog
...
/**
TSreadRiverWare_JDialog constructor.
...
{	String file = __file_JTextField.getText().trim();
	...
	String routine = "TSreadRiverWare_JDialog.checkInput";
	...
	try {
		...
		if ( !f.exists() ) {
			warning +=
			"The RiverWare file does not exist:\n" +
			"    " + adjusted_path + "\n" +
		  	"Correct or Cancel.";
		}
		f = null;
...
{	String alias = "";
	...
	if ( __first_time ) {
		...
		if ( ((String)__command_Vector.elementAt(0)).indexOf('=')>=0){
			...
			// Assume TS X =readRiverWare(file,TSID,units,start,end)
...
*/
				if ( (v != null) && (v.size() >= 3) ) {
					// Third field is the requested units.
					// Currently ignored.
				}
				if ( (v != null) && (v.size() >= 4) ) {
					// Fourth field is read start...
					analysis_period_start =
