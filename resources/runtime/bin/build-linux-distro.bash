...
getTstoolVersion() {
	local version
	version=$(${binFolder}/tstool --version | grep -i 'TSTool version' | cut -d ':' -f 2 | awk '{$1=$1};1' | cut -d ' ' -f 1)
...
printUsage () {
	...
	echo ""
	echo "Example:"
	echo "  ${scriptName} -s TSTool-linux-13.00.00.sh"
	echo "  ${scriptName} -s TSTool-jre-linux-13.00.00.sh  (if JRE is packaged)"
	echo ""
	...
	echo "                              The default is:  TSTool-linux-NN.NN.NN.run"
