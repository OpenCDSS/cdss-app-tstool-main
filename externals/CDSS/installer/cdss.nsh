...
# - for example the 'installer/CDSS/TSTool.nsi' file defines preprocessor
#   values described below and then includes this file
#
# DISPLAYNAME - a nice display name, only used for text, etc. (e.g., "TSTool")
# CODENAME[optional, defaults to DISPLAYNAME] - a "legal" name - should not have
#   special characters, etc. - used for paths, exe, etc.
# VERSION - some versioning string (e.g., "14.8.0")
#
...
# Prior to TSTool 14.8.0 used this.
#!define COMPANY OWF
...
# - for example C:\CDSS\TSTool-14.8.0
...
#InstallDirRegKey $registryTree "${REGKEY}" Path
...
Section -setInstallVariables
    ...
    # As of TSTool 14.8.0 don't require admin rights so the default is to write to the current user registry tree:
...
########################################################
Section -Main
    # As of TSTool 14.8.0 don't require admin rights so the default is to write to the user registry.
    #WriteRegStr $registryTree "${REGKEY}\Components" Main 1
...
Section "${CODENAME}" ${CODENAME}
    ...
    #WriteRegStr $registryTree "${REGKEY}" Path $INSTDIR
    #WriteRegStr $registryTree "${REGKEY}" StartMenuGroup $StartMenuGroup
    ...
    # As of TSTool 14.8.0 don't require admin rights so the default is to write to the current user registry tree.
    #WriteRegStr $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}" DisplayName "${NAMEVERSION}"
    #WriteRegStr $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}" DisplayVersion "${VERSION}"
    #WriteRegStr $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}" Publisher "${COMPANY}"
    #WriteRegStr $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}" URLInfoAbout "${URL}"
    #WriteRegStr $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}" DisplayIcon $INSTDIR\Uninstall_${NAMEVERSION}.exe
    #WriteRegStr $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}" UninstallString $INSTDIR\Uninstall_${NAMEVERSION}.exe
    #WriteRegDWORD $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}" NoModify 1
    #WriteRegDWORD $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}" NoRepair 1
...
#
#  CDSS / TSTool-Version -> run TSTool
#  
##############################################
Section "Start Menu" StartMenu
    ...
    # - the shortcut is created in C:\ProgramData\Microsoft\Windows\Start Menu\Programs\CDSS\TSTool-14.8.0.lnk
...
Section "Uninstall"
    ...
    #DeleteRegKey $registryTree "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}"
    DeleteRegKey HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${NAMEVERSION}"
    # Comment the following since uninstall menu link should no longer be added as of 2018-09-14.
    #Delete /REBOOTOK "$SMPROGRAMS\$StartMenuGroup\Uninstall\${NAMEVERSION}.lnk"
    # Comment the following since documentation menu link should no longer be added as of 2018-09-14.
    ...
    # TODO smalers 2023-04-30 the following does not seem to remove enough (leaves Components?)
    # and therefore is still shown in Windows Add/Remove programs.
    # As of TSTool 14.8.0 don't require admin rights so the default is to write to the current user registry tree.
    # REGKEY includes TSTool-14.8.0, for example.
    #DeleteRegValue $registryTree "${REGKEY}" StartMenuGroup
    #DeleteRegValue $registryTree "${REGKEY}" Path
    #DeleteRegKey /IfEmpty $registryTree "${REGKEY}\Components"
    #DeleteRegKey /IfEmpty $registryTree "${REGKEY}"
    ...
    # As of TSTool 14.8.0 don't require admin rights so the default is to write to the current user registry tree.
    #DeleteRegValue $registryTree "${REGKEY}\Components" Main
